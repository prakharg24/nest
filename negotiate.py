...
from task_utils import get_chunks
...
agent1 = RandomAgentConsiderate(score_weightage, length_penalty, 4)
agent2 = RandomAgentStubborn(score_weightage, length_penalty, 9)
...
for round in range(num_rounds):
    ...
    for agent_tuple in chunk_list:
        ...
        end_deal = False
        while not end_deal:
            prev_emotion, prev_intent, prev_proposal, end_deal = agent_tuple[act_ag].step(prev_emotion, prev_intent, prev_proposal)
            exit()
            act_ag = (act_ag+1)%2
